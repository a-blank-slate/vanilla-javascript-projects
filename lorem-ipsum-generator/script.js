...
$(document).ready(function(){
    ...
    $('#generate-button').on('click', function() {
        var enteredNumber = $('#generate-number').val();
        ...
        if(isNaN(enteredNumber) || enteredNumber <= 0 ){
            ...
            for(i=0 ;i <=enteredNumber;i++){
                ...
                finalText += `<p class="alert alert-primary"> ${randomText[randomNumber]} </p>`;
            }
            $('#generate-text').html(finalText);
        }
    });	
    
    function putRandomString(){
        var randomNumber = Math.floor(Math.random()*9);
        $('#generate-text').html(`<p class="alert alert-primary result">${randomText[randomNumber]} </p>`);
    }
});
